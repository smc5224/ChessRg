...
def detect_moves(initial_image, new_image):
    ...
    if len(moves) == 2:
        ...
        if piece_A is not None and piece_B is None:
            if is_valid_move(piece_A, Acell, Bcell) == True:
                if ((piece_A=='WP' and Bcell[0] == 0) or (piece_A=='BP'and Bcell[0] == 7)): #프로모션 조건 확인
                    ...
                    # A에 기물이 있고 B는 비어 있는 경우
                    test.append(f"{piece_A}가 {Acell}에서 {Bcell}로 이동했습니다.")
                    board_state[Acell[0]][Acell[1]] = None  # A 위치를 빈칸으로
                    board_state[Bcell[0]][Bcell[1]] = piece_A  # B 위치로 이동
            else : 
                test.append(f"규칙오류 : {piece_A}가 {Acell}에서 {Bcell}로 이동할 수 없습니다.")
                turn_count-=1
            ...
            if is_valid_move(piece_B, Bcell, Acell) == True:
                ...
                # B에 기물이 있고 A는 비어 있는 경우
                    test.append(f"{piece_B}가 {Bcell}에서 {Acell}로 이동했습니다.")
                    board_state[Bcell[0]][Bcell[1]] = None  # B 위치를 빈칸으로
                    board_state[Acell[0]][Acell[1]] = piece_B  # A 위치로 이동
            else : 
                test.append(f"규칙오류 : {piece_B}가 {Bcell}에서 {Acell}로 이동할 수 없습니다.")
                turn_count-=1
            ...
            if is_valid_move(start_piece, startCell, endCell) == True:
                ...
                if ((start_piece=='WP' and endCell[0] == 0) or (start_piece=='BP'and endCell[0] == 7)): #프로모션 조건 확인
                    ...
                    if turn_count % 2 != 0:  # 홀수 턴이면 백이 흑을 잡음
                        test.append(f"백의 {start_piece}가 {endCell}에서 흑의 {end_piece}를 잡았습니다.")
                    else:  # 짝수 턴이면 흑이 백을 잡음
                        test.append(f"흑의 {start_piece}가 {endCell}에서 백의 {end_piece}를 잡았습니다.")
                    board_state[endCell[0]][endCell[1]] = start_piece  # B 위치로 A의 기물 이동
                    board_state[startCell[0]][startCell[1]] = None  # A 위치를 빈칸으로

            else :
                    if turn_count % 2 != 0:  # 홀수 턴이면 백이 흑을 잡음
                        test.append(f"규칙오류 : 백의 {piece_A}가 {Bcell}에서 흑의 {piece_B}를 잡을 수 없습니다.")
                        turn_count-=1
                    else:  # 짝수 턴이면 흑이 백을 잡음
                        test.append(f"규칙오류 : 흑의 {piece_A}가 {Bcell}에서 백의 {piece_B}를 잡을 수 없습니다.")
                        turn_count-=1
        # 턴 수 증가
        turn_count += 1
        ...
        test.append("이동을 감지하지 못했습니다. 또는 복수의 이동이 감지되었습니다.")

    # print(board_state)
    return test, board_state
...
# for i in range(1, 13):
#     imageA = rf'back\uploads\test ({i}).png'  # 업로드한 체스판 이미지 경로 사용
#     imageA = detect_and_crop_chessboard(imageA)
#     imageB = rf'back\uploads\test ({i+1}).png'          # 두 번째 체스판 이미지 경로
